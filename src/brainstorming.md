...
- You do not need to prove these breakpoints, just motivate them historically.

5) Key Metrics (keep it minimal)
- For each firm-year:
  - Main metric: AUM per Employee
  - Supporting metrics:
    - Revenue per employee
    - Operating expense / AUM
    - Optional: Passive share of AUM

6) Output (what you will show)
- Figure 1 (most important): Time series of AUM per employee by firm
- Figure 2: Growth rate of AUM vs employees
- Figure 3 (optional): Passive share vs labor productivity
...
Here is the key part, something you can actually build.
...
- Limitation:
  - Vanguard Group and Fidelity Investments are private, so they are not fully in Compustat

Step 2: AUM Data (Most Important)
- Option A (best practical approach): manual + semi-automated collection from:
...
- Even rough estimates, such as ETFs versus total AUM, are enough for the purpose.
...
- You can do everything in pandas:
  ...
  - Compute `df["aum_per_employee"] = df["AUM"] / df["EMP"]`
  - Plot trends with matplotlib

9) What Makes This Work
- You are not claiming causality.
- You are showing stylized facts.
- You connect:
  - Technology -> scalability -> labor efficiency

10) Expected Findings (What you will likely see)
...
- Compare passive-heavy firms vs active-heavy firms
- Show Vanguard vs Fidelity divergence
